    ...
    public partial class Ejercicios : Form
    {
        ...
        private void BGuardarEjercicio_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (objEjercicio.id_ejercicio == 0) //crea un ejercicio nuevo
                {
                ...
                else //edicion de un ejercico
                {
                    int ejercicioEditado = objCN_Ejercicio.Editar(objEjercicio, out mensaje);

                    if (ejercicioEditado == 1)
                    {
                        // Mensaje de éxito
        ...
        private void dataGridViewEjercicios_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dataGridViewEjercicios.Columns["Editar"].Index && e.RowIndex >= 0)
            {
                ...
                DataGridViewRow filaSeleccionada = dataGridViewEjercicios.Rows[e.RowIndex];

                // Pasa los valores de la fila seleccionada a los TextBox
                textBoxID.Text = filaSeleccionada.Cells["idEjercicio"].Value.ToString();
                textBoxNombreEjercicio.Text = filaSeleccionada.Cells["nombre"].Value.ToString();
                textBoxRepeticiones.Text = filaSeleccionada.Cells["repeticiones"].Value.ToString();
                textBoxTiempo.Text = filaSeleccionada.Cells["tiempo"].Value.ToString();
            }

            //verifica que hizo click en la columna eliminar
            if (e.ColumnIndex == dataGridViewEjercicios.Columns["eliminar"].Index && e.RowIndex >= 0)
            {
                int idEjercicio = Convert.ToInt32(dataGridViewEjercicios.Rows[e.RowIndex].Cells["idEjercicio"].Value);

                string mensaje = string.Empty;

                DialogResult result = MessageBox.Show("¿Estás seguro de que deseas eliminar este ejercicio?", "Confirmar eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if (result == DialogResult.Yes)
                {
                    int respuesta = new CN_Ejercicio().Eliminar(idEjercicio, out mensaje);
